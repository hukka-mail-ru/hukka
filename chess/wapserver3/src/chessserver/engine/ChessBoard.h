...
public:
    ...
    int in_progress;               // game in progress flag
    operator string() const;
    ChessBoard();
    virtual ~ChessBoard();
    void print(ostream *os) const;
    bool can_move(int from, int to, piece_type promotion=Empty) const;
    bool can_move(const SMove &m) const
        {return can_move(m.from, m.to, m.promotion);}
    ChessGameStatus do_move(int from, int to, piece_type promotion=Empty);
    ChessGameStatus do_move(const SMove& m)
    ...
    void setPosition( SPosition* _pCurPos );
...
private:
    ...
    bool no_pieces_for_mate(void) const;
    bool is_attacked(bool by_white, int to) const;
    bool causes_check(const SMove& m) const;
...
inline bool ChessBoard::is_dest(int from, int to) const {
    return is_on_board(to) &&
        (is_white(m_CurPos.square[from]) && !is_white(m_CurPos.square[to]) ||
         is_black(m_CurPos.square[from]) && !is_black(m_CurPos.square[to]));
}
