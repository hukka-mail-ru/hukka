...
{	
	...
	openlog( "gammonserver", LOG_CONS, LOG_LOCAL0 );
	CGammonService* pGammonService = CGammonService::Instance();

	CConnector Connector( static_cast<ISocketManager*>( pGammonService ) );

	if( !Connector.Connect( 1234, "localhost", static_cast<CAccessInfo*>( pGammonService ) ) )
	{
